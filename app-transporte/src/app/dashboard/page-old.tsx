...
export default function Dashboard() {
  ...
  const [userInfo, setUserInfo] = useState<{email: string; role: string; motorista?: {id: string, nome: string}} | null>(null)
    ...
    async function carregarUserInfo() {
      try {
        ...
        if (res.ok) {
          ...
          // Se for motorista, carregar apenas seus dados
          if (user.role === 'MOTORISTA' && user.motorista?.id) {
            // Redirecionar para o dashboard específico do motorista
            router.push(`/dashboard/motorista/${user.motorista.id}`)
            return
          }
    ...
    <Layout>
      <div className="space-y-6">
        {loading ? (
          ...
          <>
            {/* Dashboard do Administrador da Transportadora */}
              ...
              <Card>
                ...
                <CardContent>
                  {loading ? (
                    ...
                    <div className="space-y-3">{/* existing motoristas list code */}
                      ...
                      <div className="flex justify-between items-start">
                        ...
                        <div className="text-right flex flex-col items-end gap-2">
                          ...
                          <Button 
                            size="sm" 
                            variant="outline"
                            onClick={() => router.push(`/dashboard/motorista/${motorista.id}`)}
                            className="text-xs"
                          >
                            Ver Detalhes
                          </Button>
